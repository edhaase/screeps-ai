...
/**
 * Seek: Move to the source.
 */
module.exports.seek = function(creep, flag) {
	if(!creep.pos.isNearTo(flag.pos))
		creep.moveTo(flag.pos, {reusePath: 5}); // switch to better pathfinding?
	else {		
		creep.memory.travelTime = CREEP_LIFE_TIME - creep.ticksToLive;			
		creep.say('arrived!');
		// find what we're mining, and pick a state
		var parts = flag.name.split("_");
		var sourceId = parts[1];	
 ...
 * Mine: Harvest energy
 */
module.exports.mine = function(creep, flag, source) {
	var parts = flag.name.split("_");
	var sourceId = parts[1];	
	source = Game.getObjectById(sourceId); 
