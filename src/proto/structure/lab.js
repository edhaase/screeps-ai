...
const { runReaction, boostCreep, unboostCreep, reverseReaction } = StructureLab.prototype;
...
StructureLab.prototype.runReaction = function (lab1, lab2) {
	...
	Log.debug(`${this.pos} runReaction status ${status} on tick ${Game.time}`, 'Lab');
	if (status === OK) {
		this.busy = true;
	}
	return status;
...
StructureLab.prototype.boostCreep = function (creep, bodyPartsCount) {
	...
	Log.debug(`${this.pos} boostCreep on ${creep.name}/${creep.pos} with ${this.mineralType} status ${status} on tick ${Game.time}`, 'Lab');
	if (status === OK)
		this.busy = true;
	return status;
};

StructureLab.prototype.unboostCreep = function (creep) {
	if (this.busy)
		return ERR_BUSY;
	const status = unboostCreep.apply(this, arguments);
	Log.debug(`${this.pos} unboostCreep on ${creep.name}/${creep.pos} status ${status} on tick ${Game.time}`, 'Lab');
	return status;
};

StructureLab.prototype.reverseReaction = function (creep) {
	if (this.busy)
		return ERR_BUSY;
	const status = reverseReaction.apply(this, arguments);
	Log.debug(`${this.pos} reverseReaction with ${this.mineralType} status ${status} on tick ${Game.time}`, 'Lab');
	return status;
};