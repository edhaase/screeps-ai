...
StructureNuker.prototype.run = function () {
	...
	if (this.failsafeTriggered()) {
		Log.notify(`Nuker ${this.pos.roomName} failsafe trigger! Would strike: ${this.memory.failsafe}`);
		// @todo FIRE A NUKE!
		this.defer(150);
	}
...
StructureNuker.prototype.failsafeTriggered = function () {
	const { controller } = this.room;
	if (!this.memory.failsafe)
		return false;	// Can't failsafe if we don't have a target.
	if (controller.level === 1 && this.ticksToDowngrade < 500)
		return true;
	const imminentDange = !_.isEmpty(this.pos.findInRange(this.room.hostiles, PANIC_RANGE));
	if (_.all(this.room.hostiles, 'owner.username', INVADER_USERNAME))
		return false;
	if (imminentDange && this.hitsEffective < IGNORE_IF_HP_OVER)
		return true;
	return false;
};
