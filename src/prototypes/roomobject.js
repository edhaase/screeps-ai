...
RoomObject.prototype.hasEffect = function (pwr, filter = () => true) {
	return !!(this.effects && this.effects.some(e => e.effect === pwr && filter(e)));
}

RoomObject.prototype.say = function (msg, color = 'yellow') {
	const { x, y, roomName } = this.pos;
	const newPos = new RoomPosition(x, y - 0.75, roomName);
	const visual = (this.room && this.room.visual) || new RoomVisual(roomName);
	visual.text(msg, newPos, { color, stroke: 'black' });
};