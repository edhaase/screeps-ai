...
class LivingEntity extends RoomObject {
	...
	runFleeRoom({ room, range = 5 }) {
		...
		if (this.heal)
			this.heal(this);
		const hostiles = _.filter(this.room.hostiles, unauthorizedCombatHostile);
	...
	runMoveTo(opts) {
		...
		if (this.pos == null || this.pos.inRangeTo(roomPos, opts.range) || opts.failed >= MOVE_STATE_FAILED_ATTEMPTS) // Should happen before fatigue, in case we've arrived
		...
		if (status === ERR_NO_PATH) {
			...
			Log.error(`${this.name}/${this.pos} failed to move to ${roomPos} range ${opts.range}, status ${status}`, 'LivingEntity');
		} else if (status !== OK) {
	...
	moveToRoom(roomName, enter = true) {
		if (this.pos.roomName === roomName && (!enter || !this.pos.isOnRoomBorder()))
			return ERR_NO_PATH;
		// if (enter && !this.pos.isOnRoomBorder()))
	...
	runHealSelf(opts) {
		...
		if (this.hasActiveBodypart && this.hasActiveBodypart(HEAL) && this.hitPct > 0.50) {
			...
			if (opts.hits != null) {
				...
				this.log(`hit move avg: ${opts.hma}`, LOG_LEVEL.INFO);
			}
			opts.hits = this.hits;
			// Flee was here
			return;
		...
		const target = this.getTarget(
			...
			(c) => loadedTower(c) || c.getRole && (c.getRole() === 'healer' || c.getRole() === 'guard') && c.hasActiveBodypart(HEAL),
			(candidates) => {
		...
		if (!target) {
			...
			if (status !== OK)
				Log.warn(`${this.name}/${this.pos} moving to target ${target}/${target.pos} range ${range}, status ${status}`, 'LivingEntity');
		}
	}
	...
	runUnload(opts) {
		...
		if (terminal && this.pos.isNearTo(terminal)) {
			...
			const goal = this.pos.findClosestTerminal(true);
			if (goal)
	...
	runWithdraw(opts) {
		...
		if (amount <= 0 || !target.pos.hasWithdrawAccess())
