...
const { ThreadPoolExecutor } = require('os.core.executor.threadpoolexecutor');
...
const MINIMUM_RESERVER_COST = require('role.reserver').minCost;
 ...
 * }
 */
class RemoteProc extends Process {
	constructor(opts) {
		super(opts);
	...
	*mineRoom(spawnRoom, sourceRoom, sources) {
		...
		if (controller) {
			...
			if (controller.owner) {
				...
				this.evict(sources);
				return;
			} else if (controller.reservation && Player.status(controller.reservation.username) >= PLAYER_NEUTRAL && controller.reservation.username !== WHOAMI) {
	...
	*mine(source) {
	...
	onStart() {
		...
		for (const [name, flag] of Object.entries(Game.flags)) {
			...
			if (flag.color !== FLAG_MINING || flag.secondaryColor === SITE_PICKUP)
				continue;
