...
class WorldPosition {
	...
	findRouteToWorldPosition(pos, opts) {
		return Game.map.findRoute(this.roomName, pos.roomName, opts);
	}

	findPathToWorldPosition(pos, opts) {
		const src = this.toRoomPosition();
		const dst = pos.toRoomPosition();
	...
	isHighway() {
		const parsed = this.roomName.match(/^[WE]([0-9]+)[NS]([0-9]+)$/);
	...
	static fromRoomPosition(roomPos) {
		...
		let [, h, wx, v, wy] = roomName.match(/^([WE])([0-9]+)([NS])([0-9]+)$/);
		if (h === 'W') wx = ~wx;
		if (v === 'N') wy = ~wy;
...
module.exports.WorldPosition = WorldPosition;