 ...
 * 
 * @auth eduter
 * https://github.com/eduter/render-intel-map
 */
'use strict';

const MY_USERNAME = WHOAMI;
...
export function renderIntelMap(targetRoom, roomsInfo, options = {}) {
	...
	for (let row = 0; row < rows; row++) {
		for (let col = 0; col < cols; col++) {
			...
			drawRoom(roomsInfo[roomName], roomName, lastVisitThreshold, rv, roomCenterX, roomCenterY, innerSize, opacity, boundRenderBehind, options.colorFn);
			if (displayExits) {
